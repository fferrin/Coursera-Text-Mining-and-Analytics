...
class PLSA {
 private:
  ...
  double
      lambda_B;  // Coefficient that controls the proportion of background words
 ...
 public:
  PLSA(std::shared_ptr<index::inverted_index> index,
       std::map<int, int>& inv_term_index, std::vector<std::vector<int>>& C,
       std::vector<double>& PCollection, int k, int iters, double lambda,
       double random_seed, bool submit);  // Constructor
  void Estep();
  void Mstep();
  void Iterate();  // Keeps calling the E and M steps until the required number
...
void PLSA::Estep() {
  ...
  for (int d = 0; d < num_docs; ++d)  // Loop over documents
  {
    for (int w = 0; w < num_words; ++w)  // Loop over words
    {
      ...
      for (int j = 0; j < num_topics;
           ++j)  // Loop over topics and calculate the numerator for each topic
      {
      ...
      if (denominator == 0) {
        denominator = 10e-40;
      }  // Ignore this (avoids division by zero)
      for (int j = 0; j < num_topics;
           ++j)  // Normalize the numerator of each topic
  ...
  for (int d = 0; d < num_docs; ++d)  // Loop over documents
  {
    for (int w = 0; w < num_words; ++w)  // Loop over words
    {
      ...
      for (int j = 0; j < num_topics; j++)
        PzB[d][w] = pi[d][j] * Pw[j][w];

      PzB[d][w] = lambda_B * PB[w] / (lambda_B * PB[w] + (1 - lambda_B)*PzB[d][w]);
    }
  }
}
...
void PLSA::Mstep() {
  ...
  for (int d = 0; d < num_docs; ++d)  // Loop over documents
  {
    ...
    for (int j = 0; j < num_topics;
         ++j)  // Normalize the numerator of each mixing weight
  ...
  for (int j = 0; j < num_topics; ++j)  // Loop over topics
  {
    ...
    for (int w = 0; w < num_words;
         ++w)  // Normalize the numerator of each P(w|theta_j)
...
void PLSA::Iterate() {
  for (int i = 0; i < iterations; ++i) {
    Estep();
    Mstep();
    if (!submission) PrintLikelihood(i);
  }
  if (!submission)
    PrintTopics();
  else
...
void PLSA::PrintLikelihood(int i) {
  ...
  for (int d = 0; d < num_docs; ++d) {
    for (int w = 0; w < num_words; ++w) {
      double sum = 0;
      for (int j = 0; j < num_topics; ++j) sum += pi[d][j] * Pw[j][w];
      loglikelihood +=
          Counts[d][w] * log(lambda_B * PB[w] + (1 - lambda_B) * sum);
    }
  }
  std::cout << "Iteration: " << i << ". Log-Likelihood= " << loglikelihood
            << std::endl;
}
...
void PLSA::PrintTopics() {
  for (int i = 0; i < num_topics; ++i) {
    ...
    for (int j = 0; j < num_words; ++j) indices[j] = j;
    std::sort(indices.begin(), indices.end(),
              [&](size_t x, size_t y) { return Pw[i][x] > Pw[i][y]; });
    std::cout << printing::make_bold("============================")
              << std::endl;
    std::cout << printing::make_bold("Topic: ") << i + 1 << std::endl;
    for (int j = 0; j < 20; ++j)
      std::cout << idx->term_text(inverse_term_index[indices[j]]) << " "
                << Pw[i][indices[j]] << std::endl;
  }
}

// Constructor
PLSA::PLSA(std::shared_ptr<index::inverted_index> index,
           std::map<int, int>& inv_term_index, std::vector<std::vector<int>>& C,
           std::vector<double>& PCollection, int k, int iters, double lambda,
           double random_seed, bool submit) {
  idx = index;
  num_topics = k;
  iterations = iters;
  lambda_B = lambda;
  ...
  for (int i = 0; i < num_docs; ++i) {
    ...
    for (int j = 0; j < num_words; ++j) Pz[i][j].resize(num_topics);
  }

  PzB.resize(num_docs);
  for (int i = 0; i < num_docs; ++i) PzB[i].resize(num_words);
  ...
  for (int i = 0; i < num_docs; ++i) {
    ...
    for (int j = 0; j < num_topics; ++j) {
      if (!submission)
        ...
        pi[i][j] =
            i + j;  // Use non-random initialization in case of submission
      total += pi[i][j];
    }
    for (int j = 0; j < num_topics; ++j) pi[i][j] = pi[i][j] / total;
  }
  ...
  for (int i = 0; i < num_topics; ++i) {
    ...
    for (int j = 0; j < num_words; ++j) Pw[i][j] = Pw[i][j] / total;
  }
}
...
void PLSA::Submit() {
  ...
  if (!submission.is_open())
    std::cout << "Problem writing the output to the system. Make sure the "
                 "program has enough writing privileges."
              << std::endl;
  for (int i = 0; i < num_topics; ++i) {
    std::vector<size_t> indices(num_words);
    for (int j = 0; j < num_words; ++j) indices[j] = j;
    std::sort(indices.begin(), indices.end(),
              [&](size_t x, size_t y) { return Pw[i][x] > Pw[i][y]; });
    for (int j = 0; j < 20; ++j)
      submission << idx->term_text(inverse_term_index[indices[j]]) << " "
                 << Pw[i][indices[j]] << std::endl;
  }
  submission.close();
}
...
int main(int argc, char* argv[]) {
  if (argc != 10 && argc != 3) {
    std::cerr << "Error: Usage:\t" << argv[0] << " configFile "
              << "--topics "
              << "--lambda "
              << "--iter "
              << "--seed" << std::endl;
  ...
  auto idx = index::make_index<index::dblru_inverted_index>(
      *config, 30000);  // Inverted Index

  auto fidx = meta::index::make_index<index::memory_forward_index>(
      *config);  // Forward Index
  ...
  for (size_t i = 0; i < idx->num_docs(); ++i) {
    C[i].resize(j);
    auto postings_list =
        fidx->index::forward_index::search_primary(i)->counts();
  ...
  PLSA X(idx, inverse_term_index, C, PCollection, k, iter, lambda, seed,
         argc == 3);
  X.Iterate();
  return 0;
}
