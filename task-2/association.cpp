...
double MutualInformation(int Na, int Nb, int Nab, int N) {
  ...
  */
  ...
  double PXab01 = PXb1 - PXab11;  // P(Xw_{a} = 0, Xw_{b} = 1)
  double PXab10 = PXa1 - PXab11;  // P(Xw_{a} = 1, Xw_{b} = 0)
  double PXab00 = PXa0 - PXab01;  // P(Xw_{a} = 0, Xw_{b} = 0)
...
struct compare {
  bool operator()(const std::tuple<double, int, int> l,
                  const std::tuple<double, int, int> r) {
...
The queue has the lowest value at the head; Every new value (co-occurrence or
MI)
...
std::priority_queue<std::tuple<double, int, int>,
                    std::vector<std::tuple<double, int, int>>, compare>
top_terms(std::shared_ptr<index::inverted_index> idx, bool MI,
          size_t num_terms) {
  std::priority_queue<std::tuple<double, int, int>,
                      std::vector<std::tuple<double, int, int>>, compare>
      A;
  for (size_t i = 0; i < idx->unique_terms();
       ++i)  // Loop over first word in the pair
  {
    if (idx->doc_freq(i) > 20)  // Ignore words with document frequency below 20
    {
      auto postings_list1 =
          idx->search_primary(i)
              ->counts();  // Extract the documents that have the first term
      for (size_t j = i + 1; j < idx->unique_terms();
           ++j)  // Loop over second word in the pair
      {
        if (idx->doc_freq(j) >
            20)  // Ignore words with document frequency below 20
        {
          auto postings_list2 =
              idx->search_primary(j)->counts();  // Extract the documents that
                                                 // have the second term
          int num_occurences = 0;  // Number of co-occurrences of the pair (i,j)
          // Take the intersection of the two postings lists. The size of the
          // intersection is the number of co-occurrences.
          ...
          double score;
          if (MI == 1)
            score = MutualInformation(idx->doc_freq(i), idx->doc_freq(j),
                                      num_occurences, idx->num_docs());
          else
...
int main(int argc, char* argv[]) {
  ...
  while (i < argc) {
    try {
      if (std::strcmp(argv[i], "--words") == 0) {
        num_words = std::atoi(argv[i + 1]);
        i += 2;
      } else if (std::strcmp(argv[i], "--MI") == 0) {
        MI = 1;
        i += 1;
        ...
        if (!submission.is_open())
          std::cout << "Problem writing the output to the system. Make sure "
                       "the program has enough writing privileges."
                    << std::endl;
      } else
        throw 1;
    } catch (...) {
  ...
  if (!submit)
    for (size_t j = 0; j < v.size(); ++j)
      std::cout << std::get<0>(v[j]) << " " << idx->term_text(std::get<1>(v[j]))
                << " " << idx->term_text(std::get<2>(v[j])) << std::endl;

  else {
    for (size_t j = 0; j < v.size(); ++j)
      submission << std::get<0>(v[j]) << " "
                 << idx->term_text(std::get<1>(v[j])) << " "
                 << idx->term_text(std::get<2>(v[j])) << std::endl;
    submission.close();
  }
